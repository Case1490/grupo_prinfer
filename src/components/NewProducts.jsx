...
const NewProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const querySnapshot = await getDocs(collection(db, "productos_del_mes"));
  ...
  return (
    <div>
      <div className=" flex flex-col space-y-4 sm:flex-row sm:justify-between sm:items-center">
        ...
        <Link
          to="/novedades"
          className=" text-center text-xl rounded-full border-2 py-2 px-4 border-BlueDark hover:bg-BlueDark hover:text-white"
        >
          Ver todo
        </Link>
      </div>
